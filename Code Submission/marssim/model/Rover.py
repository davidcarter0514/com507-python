...
class Rover(Agent):
    ...
    def act(self,mars:Mars):
        ...
        if self.__status == "Deadlocked":
            ...
            if rock_agents:
                ...
                # move towards target rock
                if not (self.__target_rock_location and self.__move_towards_location(self.__target_rock_location, mars, self.get_remaining_out_movement(mars)-1)):
            ...
            # Contract Supply
                ...
                if self.__rock != None or self.get_remaining_out_movement(mars) < 1:
                    self.__return_home(mars)
                elif mars.distance_between_locations(self.get_location(), self.__contract.get_recharge_point()) <= 1:
                    self.__return_home(mars)
    ...
    def __scan_for_rovers(self, mars:Mars):
        ...
        if rovers_found:
            for rover in rovers_found:
                ...
                if self.__target_rock_location and rover.get_target_rock_location() and mars.compare_locations(self.__target_rock_location, rover.get_target_rock_location()):
                    self.set_target_rock_location(mars)
                ...
                if rover.get_contract() != None and self.__contract != None and rover.get_contract() != self.__contract and mars.compare_locations(rover.get_contract().get_recharge_point(),self.__contract.get_recharge_point()):
                    ...
                    if self.__contract.get_reached_rcp() and not rover.get_contract().get_reached_rcp():
                        break

                    print("Changing recharge point")
                    near_locations = mars.available_ring_locations(self.__spaceship_location,self.__one_rover_movement + 1)

                    memory = self.__memory.get_entity_list_of_none(self.get_location())

                    loc_dist = []
                    for loc in near_locations:
                        for loc_memory in memory:
                            if mars.compare_locations(loc,loc_memory[0].get_location()):
    ...
    def __move_towards_location(self, target_location:Location, mars:Mars,remaining_movement:int):
        ...
        if free_locations:
            ...
            for entry in sorted_locations:
                if entry[1] <= remaining_movement:
                    try_move = self.__move(entry[0],mars)
                    if try_move:
                        self.__deadlock_counter = 0
                        break
            else:
        ...
        for info in new_info:
            ...
            for scanned_entity in known_entities:
                ...
                if mars.compare_locations(info.get_location(), scanned_entity.get_location()):
                    ...
                    # Check the conflicting entities resolve None > Rock, otherwise keep memory the same.
                        if scan_entity == None:
                            add_to_memory = False
    ...
    def __deadlock_protocol(self,mars:Mars):
        ...
        if self.__contract != None and self.__contract.get_master() == self:
            ...
            for entry in ss_loc:
                try_move  = self.__move_towards_location(entry[0],mars,self.get_remaining_movement()-1)
                if try_move:
                    break
            else:
                self.__move_towards_location(self.__spaceship_location, mars, self.get_remaining_movement())
